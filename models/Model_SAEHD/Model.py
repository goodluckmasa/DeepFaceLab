            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/GPU:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    if self.options['ms_ssim_loss']:
                        gpu_src_loss = 10 * nn.PercpLoss(resolution)(gpu_target_src_masked_opt, gpu_pred_src_src_masked_opt, max_val=1.0)
                    else:
                        if resolution < 256:
                    ...
                    if self.options['background_power'] > 0:
                        ...
                        if self.options['ms_ssim_loss']:
                            gpu_src_loss += bg_factor * 10 * nn.PercpLoss(resolution)(gpu_target_src, gpu_pred_src_src, max_val=1.0)
                        else:
                            if resolution < 256:
                    ...
                    if self.options['ms_ssim_loss']:
                        gpu_dst_loss = 10 * nn.PercpLoss(resolution)(gpu_target_dst_masked_opt, gpu_pred_dst_dst_masked_opt, max_val=1.0)
                    else:
                        if resolution < 256:
                    ...
                    if self.options['background_power'] > 0:
                        bg_factor = self.options['background_power']
                        if self.options['ms_ssim_loss']:
                            gpu_dst_loss += bg_factor * 10 * nn.PercpLoss(resolution)(gpu_target_dst, gpu_pred_dst_dst, max_val=1.0)
                        else:
                            if resolution < 256:
