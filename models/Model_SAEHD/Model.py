        ...
        if self.is_training:
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/GPU:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    if eyes_mouth_prio:
                        if self.options['ms_ssim_loss']:
                            gpu_src_loss += 300 * nn.MsSsim(resolution, kernel_size=5)(gpu_target_src*gpu_target_srcm_em, gpu_pred_src_src*gpu_target_srcm_em, max_val=1.0)
                        else:
                    ...
                    if eyes_mouth_prio:
                        if self.options['ms_ssim_loss']:
                            gpu_dst_loss += 300 * nn.MsSsim(resolution, kernel_size=5)(gpu_target_dst*gpu_target_dstm_em, gpu_pred_dst_dst*gpu_target_dstm_em, max_val=1.0)
                        else:
