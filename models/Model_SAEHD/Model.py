            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/GPU:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    if self.options['ms_ssim_loss']:
                        ...
                        gpu_src_loss =  tf.reduce_mean ( 10*(1-(tf.image.ssim_multiscale(gpu_target_src_masked_opt, gpu_pred_src_src_masked_opt, max_val=1.0, power_factors=(0.05168435625288417, 0.3294877711121366, 0.34621596677434235, 0.2726119058606368))/2)))
                    else:
                        if resolution < 256:
